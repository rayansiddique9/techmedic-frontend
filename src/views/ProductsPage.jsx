...
export default function ProductsPage() {
  ...
  useEffect(() => {
    ...
    if (!rendered)
    {
      const getResponse = async () => {
        ...
        response.error ? seterror(response.error) : setProducts(response.products);
  ...
  return (
    <div className="min-h-screen">
      <Searchbar />
      <div className={`${!error ? 'grid' : 'block'}  grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 xl:gap-10 p-5`}>
